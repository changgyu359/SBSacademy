...
#pragma region Data 영역
	// 프로그램의 시작과 함께 메모리에 저장되며, 프로그램이 종료될 때 메모리가 해제되는 영역.
#pragma endregion
...
#pragma region Stack 영역
	// 프로그램이 자동으로 사용하는 임시 메모리 영역으로 함수 호출 시
	// 생성되는 지역 변수와 매개 변수가 저장되는 메모리 영역.
#pragma endregion

#pragma region Heap 영역
	// 사용자가 직접 메모리 공간을 할당해주는 메모리 영역이며, 
	// 사용하지 않는 경우 사용자가 직접 메모리를 헤제해야 되는 메모리 영역.
#pragma endregion
